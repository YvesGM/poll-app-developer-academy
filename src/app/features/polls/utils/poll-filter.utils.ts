import { Poll, PollCategory } from '../models/poll.model';
 ...
 */
export function filterPollsByCategory(polls: Poll[], category: CategoryFilter): Poll[] {
  if (category === 'All') {
    return polls;
  }
 ...
 */
export function sortPollsByDeadline(polls: Poll[]): Poll[] {
 ...
 */
export function filterEndingSoonPolls(polls: Poll[], now: number, windowMs: number): Poll[] {
 ...
 */
function comparePollDeadlines(a: Poll, b: Poll): number {
 ...
 */
function isDeadlineWithinWindow(poll: Poll, now: number, limit: number): boolean {
